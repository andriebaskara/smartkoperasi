...
class MJenisPembayaranController extends Controller
{
    public function index()
    {
        if(!Auth()->user()->hasAnyPermission('master_jenis_pembayaran'))
        {
        ...
        return view('admin.master.jenis_pembayaran.index', compact('table'));
    }
    ...
    { 
        ...
        try {        
            ...
            if ($simpan =='baru') {
                $id= 0;
                $jenis_pembayaran = M_JenisPembayaran::create($request->all());
                $pesan = "Data berhasil disimpan";

            } else {
                $id = $request->input('id');
        
                $jenis_pembayaran = M_JenisPembayaran::findOrFail($id);
                $jenis_pembayaran->update($request->all());
                $pesan = "Data berhasil diupdate";
        }

            $jenis_pembayaran->updated =  Helpers::generateUpdated();
        ...
        $table = $this::datatable($request, $jenis_pembayaran);
        $table['pesan'] = $pesan;
    ...
    public function hapusdata(Request $request)
    {
        ...
        if (! Gate::allows('master_jenis_pembayaran')) {
    ...
    public function hapusdipilih(Request $request)
    {
        ...
        if ($request->input('ids')) {
            $entries = M_JenisPembayaran::whereIn('id', $request->input('ids'))->get();
    ...
    public function datatable($request, $dataproses = null)
    {
        ...
        $skip = ($halaman - 1) * $take;
        $jenis_pembayaran = M_JenisPembayaran::where('id','!=', '0');

        if(!empty($kriteria)){
            $jenis_pembayaran = $jenis_pembayaran->where('deskripsi','LIKE','%'.$kriteria.'%');
        }

        $count = count($jenis_pembayaran->get());
        $pagination = Helpers::Pagination("LoadData", $count, $take, $halaman);
        $jenis_pembayaran = $jenis_pembayaran->skip($skip)->take($take)->get();
        ...
        foreach ($jenis_pembayaran as $detail) {
