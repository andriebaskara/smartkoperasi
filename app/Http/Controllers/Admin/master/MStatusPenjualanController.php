...
class MStatusPenjualanController extends Controller
{
    public function index()
    {
        if(!Auth()->user()->hasAnyPermission('master_status_penjualan'))
        {
        ...
        return view('admin.master.status_penjualan.index', compact('table','warna'));
    }
    ...
    { 
        ...
        try {        
            ...
            if ($simpan =='baru') {
                $id= 0;
                $status_penjualan = M_StatusPenjualan::create($request->all());
                $pesan = "Data berhasil disimpan";

            } else {
                $id = $request->input('id');
        
                $status_penjualan = M_StatusPenjualan::findOrFail($id);
                $status_penjualan->update($request->all());
                $pesan = "Data berhasil diupdate";
        }

            $status_penjualan->updated =  Helpers::generateUpdated();
            $status_penjualan->save();
            DB::commit();
        ...
        $table = $this::datatable($request, $status_penjualan);
        $table['pesan'] = $pesan;
    ...
    public function hapusdata(Request $request)
    {
        ...
        if (! Gate::allows('master_status_penjualan')) {
        ...
        if (empty($status_penjualan)) {
            return Helpers::responseJson(false, "", "Data tidak ditemukan" );
        }

        $status_penjualan->delete();
    ...
    public function hapusdipilih(Request $request)
    {
        ...
        if ($request->input('ids')) {
            $entries = M_StatusPenjualan::whereIn('id', $request->input('ids'))->get();
    ...
    public function datatable($request, $dataproses = null)
    {
        ...
        $skip = ($halaman - 1) * $take;
        $status_penjualan = M_StatusPenjualan::where('id','!=', '0');

        if(!empty($kriteria)){
            $status_penjualan = $status_penjualan->where('deskripsi','LIKE','%'.$kriteria.'%');
        }

        $count = count($status_penjualan->get());
        $pagination = Helpers::Pagination("LoadData", $count, $take, $halaman);
        $status_penjualan = $status_penjualan->skip($skip)->take($take)->get();
        ...
        foreach ($status_penjualan as $detail) {
